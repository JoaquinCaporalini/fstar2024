...
let rec fib_lin_aux (n:nat)
  : Lemma (fib_lin' n == (fib n, fib (n+1))) = 
  // admit ()
  if n = 0
  then ()
  else 
    fib_lin_aux (n - 1)
(* Demuestre que fib_lin es correcta. *)
...
let rec fib_tail_aux (n:nat) (k:nat) : Lemma (fib_tail' (fib k) (fib (k + 1)) n == fib (k + n)) =
  if n = 0
  then ()
  else
    fib_tail_aux (n - 1) (k + 1)
